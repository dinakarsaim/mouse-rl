...
class MouseEnvContinuous(gym.Env):
    ...
    def _get_obs(self, pause_flag):
        # Normalize mouse_pos and target to [0, 1]
        norm_mouse = self.mouse_pos / np.array([SCREEN_W, SCREEN_H], dtype=np.float32)
        norm_target = self.target / np.array([SCREEN_W, SCREEN_H], dtype=np.float32)
        return np.concatenate([norm_mouse, norm_target, [float(pause_flag)]], dtype=np.float32)
    ...
    def step(self, action):
        ...
        # reward = -dist/100
        reward = -dist / (np.linalg.norm([SCREEN_W, SCREEN_H]) / 2)
        ...
        return self._get_obs(pause_flag), reward, done, {}
...
def train_only(total_timesteps=1_000_000):
    env = SubprocVecEnv([lambda: MouseEnvContinuous() for _ in range(8)])
